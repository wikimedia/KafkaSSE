...
class KafkaSSE {
    ...
    constructor(req, res, options) {
        ...
        // TODO: configure matcher filter factory
    ...
    connect(assignments) {
        ...
        .then(() => {
            ...
            .catch((e) => {
                ...
                if ('sseClient' in this) {
                    utils.sseEmit(this.sseClient, 'error', e, _.values(this.latestOffsetsMap));
                }
            });
        })
    ...
    _start() {
        ...
        // Initialize the sseClient.  This will start sending
        // the response in chunked transfer encoding.
        this.sseClient = new sse.Client(this.req, this.res);
    ...
    _loop() {
        ...
        .then((kafkaMessage) => {
            ...
            // and send the kafkaMessage.message to the sseClient.
            else {
    ...
    */
    _consume() {
        ...
        // TODO: message filtering and matching.
        ...
        // Any unexpected error will be thrown to the client will not be caught here.
    ...
    */
    _deserialize(kafkaMessage) {
        return new P((resolve, reject) => {
            ...
            // TODO: fix https://github.com/Blizzard/node-rdkafka/issues/50
            // This shouldn't be needed, but until the issue is fixed, it is.
            if (!('topic' in kafkaMessage)) {
                kafkaMessage.topic = kafkaMessage.topic_name;
            }
    ...
    disconnect() {
        ...
        // If sseClient is connected, this end the response via sseClient.
        // (sseClient.close calls res.end())
        if ('sseClient' in this) {
            this.log.info('Closing SSE client.');
            this.sseClient.close();
            delete this.sseClient;
            delete this.res;
        }
