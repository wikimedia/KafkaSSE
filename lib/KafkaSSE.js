...
class KafkaSSE {
    ...
    disconnect(error) {
        ...
        if (this.is_finished) {
            this.log.info('KafkaSSE disconnect() has already been called. Doing nothing.');
            return P.resolve();
        }
        ...
        return P.resolve().then(() => {
            ...
            if (this.sse) {
                ...
                .then(() => {
                    this.log.debug('KafkaSSE disconnect: Closed SSE + HTTP response.');
                });
            }
            if (!this._resFinished()) {
                ...
                .then(() => {
                    this.log.debug('KafkaSSE disconnect: Closed HTTP request.');
                });
            }
            if (this.res) {
                delete this.res;
                this.log.debug('KafkaSSE disconnect: Deleted HTTP request.');
            }
        }).then(() => {
            ...
            .catch((e) => this._error(e, 'warn'))
            .then(() => {
                this.log.debug('KafkaSSE disconnect: Closed the Kafka consumer.');
            });
        }).then(() => {
            this.log.info('KafkaSSE disconnect finished.')
