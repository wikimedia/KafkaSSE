...
class KafkaSSE {
    ...
    connect(assignments, atTimestamp) {
        ...
        return this._init(assignments, atTimestamp)
        .then(() => {
    ...
    disconnect() {
        ...
        if (this.kafkaConsumer) {
            pKafka = this.kafkaConsumer.disconnectAsync().then(() => {
        ...
        else if (this.res && !this.res.finished) {
            pReq = new P((resolve, reject) => {
