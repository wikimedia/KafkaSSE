...
class KafkaSSE {
    ...
    connect(assignments, atTimestamp) {
        ...
        .then(() => {
            ...
            // If anything other than SSE loop errors happen during operation, emit an SSE error.
            .catch((e) => {
    ...
    _loop(startTime) {
        ...
        .then((kafkaMessage) => {
            ...
            return this.sse.send(
                ...
                // only warn on SSE send errors, these can be caused by network issues and are not
                // related to general operation of KafkaSSE
                e = this._error(e, 'warn');
                // stop the loop by not throwing or rejecting here
            }).then(() => {
